...
Objetivo: validar a estrategia em multiplos ativos para reduzir dependencia de um unico mercado.

- Suporte a multiplos symbols
- Execucao da mesma estrategia em diferentes pares
- Comparacao de desempenho entre ativos
- Validacao de robustez fora do BTC
...
- aumento de numero de trades
- reducao de dependencia de um unico ativo
- identificacao de ativos onde a estrategia funciona melhor

### Fase 7 - Portfolio Allocation

Objetivo: alocar capital dinamicamente entre os ativos que apresentam melhor desempenho.

- Alocacao percentual de capital por ativo
- Ajuste baseado em volatilidade
- Ajuste baseado em drawdown
- Possivel uso de Sharpe / retorno ajustado ao risco

Beneficios:

- melhor uso do capital
- menor drawdown agregado
- melhor estabilidade de retorno

### Fase 8 - Arbitragem Estrutural (Exploratoria)

Objetivo: estudar oportunidades de arbitragem entre mercados e pares.

Possibilidades:

- Diferencas entre BTCUSDT e BTCBRL
- Triangular arbitrage (BTC / ETH / USDT)
- Ineficiencias temporarias entre pares

Observacao:

Esta fase e exploratoria e nao e prioridade enquanto o sistema direcional ainda esta sendo consolidado.
...
Timeframe: 15m (execution) / 1h (trend filter)

Data Source:
Binance historical OHLCV

Example tested dataset window:
2022 – 2025
...
Benchmark used to contextualize strategy performance.

Example comparison structure:

Strategy:
- Profit Factor: (varia por versão)
- Expectancy: depende da otimização
- Drawdown: controlado pela estrutura de risco

Buy and Hold BTC:
- depende do período analisado
- utilizado apenas como referência macro

Observação:
O objetivo da estratégia não é necessariamente superar buy‑and‑hold em bull markets extremos, mas:

- reduzir drawdowns
- produzir retornos mais estáveis
- permitir execução sistemática
...
1. Baixar dados OHLCV do mesmo período
2. Utilizar os mesmos parâmetros de estratégia
3. Executar o backtest no mesmo timeframe
4. Comparar resultados com métricas:
...
Isso permite auditoria completa do sistema.
